...
func main() {
	...
	if err != nil {
		fmt.Printf("Error during payment request: %v\n", err)
		return
	}
	...
	// verify request
	e := bpClient.VerifyRequest(
		int64(orderId),
		int64(saleOrderId),
		int64(saleReferenceId),
		strconv.Itoa(resCode),
	)
	if e != nil {
		fmt.Printf("Error during verify request: %v\n", e)
		return
	}
}
